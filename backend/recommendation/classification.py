...
engine = create_engine("mysql+mysqlconnector://root:root@localhost:3306/law?auth_plugin=mysql_native_password")
...
df = pd.read_sql_table('vbpl_chuapd', engine)
...
for i, row in df.iterrows(): 
    ...
    try:
        ...
        df_write = pd.DataFrame([row])
        df_write.to_sql('vbpl_chuapb_v1', engine, if_exists='append', index=False) 
    except:
        continue
